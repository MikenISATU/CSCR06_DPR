...
/* ------------------ shared helpers: dedupe + table styles ------------------ */
// NEW: normalize + parse period_covered
function parsePeriods(raw?: string): string[] {
  if (!raw) return [];
  
  // First, protect date patterns like "June 29, 2025" or "Month DD, YYYY"
  // by replacing the comma+space with a placeholder
  const protectedStr = (raw || "")
    .replace(/(\b(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}),\s*(\d{4})/gi, "$1__DATECOMMA__$2")
    .replace(/(\d{1,2}\/\d{1,2}),\s*(\d{4})/g, "$1__DATECOMMA__$2");
  
  // Now split on actual separators
  const normalized = protectedStr
    .replace(/\s*(;|\/|and|&)\s*/gi, ",")
    .replace(/,+/g, ",")
    .replace(/^,|,$/g, "");
  ...
  return normalized
    .split(",")
    .map(s => s.replace(/__DATECOMMA__/g, ", ").replace(/\s+/g, " ").trim())
    .filter(Boolean);
}

// NEW: single string to store in DB (clean + dedup, nice formatting)
function formatPeriodString(raw?: string): string {
  const uniq = Array.from(new Set(parsePeriods(raw)));
  return uniq.join(", ");
}
...
function normPeriod(s: string): string {
  // Protect date commas before normalization
  const protectedStr = (s || "")
    .normalize("NFKC")
    .replace(/(\b(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}),\s*(\d{4})/gi, "$1__DATECOMMA__$2")
    .replace(/(\d{1,2}\/\d{1,2}),\s*(\d{4})/g, "$1__DATECOMMA__$2");
...
function groupAndSum(recs: Array<{ type: string; period: string; pages: number }>) {
  const byKey = new Map<string, number>();
  const typeOrder: string[] = [];
  const typeDisplay = new Map<string, string>();
  const periodIndex = new Map<string, string[]>();
  const periodDisplay = new Map<string, string>();

  recs.forEach(({ type, period, pages }) => {
    const dispType = (type || "").trim();
    const tKey = normType(dispType);

    const dispPeriod = (period || "").trim();
    const pKey = normPeriod(dispPeriod);

    const key = `${tKey}__${pKey}`;
    byKey.set(key, (byKey.get(key) ?? 0) + (Number.isFinite(pages) ? pages : 0));

    if (!typeDisplay.has(tKey)) {
      typeDisplay.set(tKey, dispType);
      typeOrder.push(tKey);
      periodIndex.set(tKey, []);
    }
    const tpKey = `${tKey}__${pKey}`;
    if (!periodDisplay.has(tpKey)) {
      periodDisplay.set(tpKey, dispPeriod);
      periodIndex.get(tKey)!.push(pKey);
    }
  });

  const result: Record<string, { period: string; pages: number }[]> = {};
  typeOrder.forEach((tKey) => {
    const prettyType = typeDisplay.get(tKey)!;
    result[prettyType] = periodIndex.get(tKey)!.map((pKey) => ({
      period: periodDisplay.get(`${tKey}__${pKey}`)!,
      pages: byKey.get(`${tKey}__${pKey}`)!,
    }));
  });
  return result;
}
...
const TD_BASE =
  "p-1 sm:p-2 text-center align-middle text-[#003087] whitespace-nowrap truncate";
const TH_BASE =
  "p-1 sm:p-2 text-center bg-[#003087] text-white sticky top-0 z-10";
const ACTION_BTN =
  "inline-flex items-center justify-center rounded hover:bg-[#F5F6F5] transition-colors p-1";
const ICON_BTN =
  "text-[#003087] hover:text-[#002060]";
const ICON_BTN_DANGER =
  "text-[#C1272D] hover:text-[#a12025]";
/* -------------------------------------------------------------------------- */
...
export default function AdminPage() {
  ...
  const [activeSlider, setActiveSlider] = useState<'users' | 'admin' | 'activity' | null>(null);
  ...
  const uniqueRoles = useMemo(() => {
    ...
    users.forEach(u => {
      ...
      if (key && key !== "ADMIN" && !seen.has(key)) seen.set(key, raw);
    });
    return Array.from(seen.values());
  }, [users]);


  // computed recent activity (latest 20 report submissions)
  ...
  useEffect(() => {
    if (user && user.role === "ADMIN") {
      async function fetchData() {
        await fetchUsers();
        await Promise.all([fetchReports(), fetchCategories(), fetchPendingRegistrations()]);
      }
      fetchData();
    }
  }, [user]);

  // Auto-close sliders when a modal opens (to avoid overlap)
  useEffect(() => {
    if (
      showPendingModal ||
      showDeleteModal ||
  ...
  async function fetchCategories() {
    const { data, error } = await supabase.from("record_categories").select("id, role, category");
    if (error) {
  ...
  async function fetchPendingRegistrations() {
    const { data, error } = await supabase.from("registration_pending").select("*");
    if (error) {
  ...
  async function addCategory() {
    ...
    const { error } = await supabase.from("record_categories").insert({ role: selectedRole, category: newCategory });
    if (error) {
  ...
  async function updateCategory(categoryId: number, updatedCategory: string) {
    ...
    const { error } = await supabase.from("record_categories").update({ category: updatedCategory }).eq("id", categoryId);
    if (error) {
  ...
  async function confirmDeleteCategory() {
    ...
    const { error } = await supabase.from("record_categories").delete().eq("id", categoryToDelete);
    if (error) {
  ...
  async function deleteUser(userId: string) {
    ...
    const { error: reportError } = await supabase.from("monthly_reports1").delete().eq("useriud", userId);
    if (reportError) {
...
async function fetchYearlyReports() {
  ...
  const formatted = (data ?? []).map((r: any) => ({
    ...
    role: r.users2?.role || "Unknown",
  }));

  setYearlyReports(formatted);
  ...
  async function deleteOldReports() {
    ...
    const { error } = await supabase.from("monthly_reports1").delete().lt("created_at", oneYearAgo.toISOString());
    if (error) {
    ...
    await fetchReports();
    if (showYearlyModal) await fetchYearlyReports();
  }
  ...
  async function approveRegistration(pendingId: string) {
    ...
    const { error: deleteError } = await supabase.from("registration_pending").delete().eq("id", pendingId);
    if (deleteError) {
  ...
  async function rejectRegistration(pendingId: string) {
    const { error } = await supabase.from("registration_pending").delete().eq("id", pendingId);
    if (error) {
  ...
  async function updateAdminCredentials() {
    ...
    const { error } = await supabase.from("users2").update({ username: adminNewUsername, password: adminNewPassword }).eq("id", user!.id);
    if (error) {
  ...
  function generateRoleReport(periodType: "monthly" | "yearly", role: string) {
    ...
    const filteredReports = reports.filter((r) => {
      ...
      if (periodType === "monthly") {
        return matchesRole && d.getFullYear() === year && d.getMonth() + 1 === month;
      } else {
        return matchesRole && d.getFullYear() === year;
      }
    });
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: { display: true, text: `Total Pages per Department (Year ${currentYear})` },
    },
    scales: {
  ...
  const monthlyChartOptions = {
    ...
    scales: {
      y: { beginAtZero: true, title: { display: true, text: "Number of Pages", font: { size: 12 } }, ticks: { font: { size: 10 } } },
      x: { title: { display: true, text: "Type of Record", font: { size: 12 } }, ticks: { font: { size: 10 }, maxRotation: 45, minRotation: 45 } },
    },
  };
  ...
  return (
    <div className="min-h-screen bg-[#F5F6F5] p-2 sm:p-4 md:p-6 relative">
      ...
      {!showCategoriesView && !showDeleteModal && !showPendingModal && !viewingDepartmentReport && !viewReport && !showYearlyModal && !showMonthlySummary && !activeSlider && !showCategoryDeleteModal && (
        <div className="hidden md:block fixed top-20 left-[calc(50%-750px)] w-48 bg-white shadow-lg rounded-lg z-40 max-h-[calc(100vh-4rem)] overflow-y-auto">
          <div className="p-3">
        ...
        <div className="md:hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex">
          <div className="w-64 bg-white h-full p-4 shadow-lg fixed top-0 left-0 max-h-full overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowMobileMenu(false)} className={`${ACTION_BTN} ${ICON_BTN}`} aria-label="Close menu" title="Close">
      ...
      <div className="w-full max-w-5xl mx-auto bg-white rounded-lg shadow-lg p-3 sm:p-4 md:p-6 mt-6 sm:mt-8 md:mt-12">
        <header className="flex flex-col sm:flex-row justify-between items-center mb-4 sm:mb-6 space-y-3 sm:space-y-0">
          <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4">
            <Image src="/logo.png" alt="Website Logo" width={120} height={40} priority className="w-24 sm:w-32 md:w-40" />
            <div className="text-center sm:text-left">
              <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-[#003087] font-['Poppins']">Admin Dashboard</h1>
              {user && (
          ...
          {/* HEADER ICON ACTIONS (icon-only + tooltip labels) */}
          <div className="flex items-center gap-2 sm:gap-3">
            {/* Pending registrations */}
            <div className="relative">
              <button
                onClick={() => { setShowPendingModal(true); setShowMobileMenu(false); }}
                className="p-2 rounded-md bg-[#003087] text-white hover:bg-[#002060] transition-colors"
              ...
              >
                ...
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  ...
                  <path d="M5 7h14l3 5v6a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-6l3-5z"/>
                </svg>
              </button>
              {pendingRegistrations.length > 0 && (
                <span className="absolute -top-2 -right-2 bg-[#C1272D] text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center">
            ...
            {/* Manage users */}
            <button
              onClick={() => setActiveSlider(activeSlider === 'users' ? null : 'users')}
              className="p-2 rounded-md bg-[#003087] text-white hover:bg-[#002060] transition-colors"
            ...
            {/* Manage admin */}
            <button
              onClick={() => setActiveSlider(activeSlider === 'admin' ? null : 'admin')}
              className="p-2 rounded-md bg-[#C1272D] text-white hover:bg-[#a12025] transition-colors"
            ...
            <button
              onClick={logout}
              className="p-2 rounded-md text-[#C1272D] hover:text-white hover:bg-[#C1272D] border border-[#C1272D] transition-colors"
              aria-label="Logout"
              title="Logout"
            >
              {/* power icon */}
                  ...
                  <table className="w-full border-collapse table-auto text-xs sm:text-sm">
                    <thead>
                      <tr className="bg-[#003087] text-white sticky top-0">
                        <th className={`${TH_BASE} w-[48px]`}>No.</th>
                        ...
                        <th className={`${TH_BASE} w-[90px]`}>Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {roleReports.map((r, index) => (
                        <tr key={r.id} className="border-b hover:bg-[#F5F6F5]">
                          <td className={`${TD_BASE}`} title={String(index + 1)}>{index + 1}</td>
                          <td className={`${TD_BASE}`} title={r.type_of_record}>{truncateLabel(r.type_of_record, 22)}</td>
                          <td className={`${TD_BASE}`} title={r.period_covered}>{truncateLabel(r.period_covered, 22)}</td>
                          <td className={`${TD_BASE}`} title={String(r.no_of_pages)}>{r.no_of_pages}</td>
        ...
        <div className="mb-6 sm:mb-8" id="manage-categories">
          ...
          <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4 mb-4">
            ...
            >
              ...
              {uniqueRoles.map((role) => (
                <option key={role} value={role} className="text-[#003087]">{role}</option>
              ))}
            </select>
            <input
              type="text"
          ...
          <div className="overflow-x-auto max-h-60">
            <table className="w-full border-collapse table-auto text-xs sm:text-sm">
              <thead>
                <tr className="bg-[#003087] text-white sticky top-0">
                  <th className={`${TH_BASE} w-[50px] text-left`}>No.</th>
                  <th className={`${TH_BASE} text-left`}>Division</th>
                  <th className={`${TH_BASE} text-left`}>Category</th>
                  <th className={`${TH_BASE} w-[110px] text-left`}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {categories.map((cat, index) => (
                  <tr key={cat.id} className="border-b hover:bg-[#F5F6F5]">
                    <td className="p-1 sm:p-2 text-[#003087]">{index + 1}</td>
                    <td className="p-1 sm:p-2 text-[#003087]">{cat.role}</td>
                    <td className="p-1 sm:p-2 text-[#003087]">
                      {editingCategory?.id === cat.id ? (
                        ...
                        <span title={cat.category} className="inline-block max-w-[28ch] truncate">{cat.category}</span>
                      )}
                    </td>
          ...
          <div className="flex flex-col space-y-3 sm:space-y-4">
            <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4">
              ...
              >
                ...
                {years.map((year) => (
                  <option key={year} value={year} className="text-[#003087]">{year}</option>
                ))}
              </select>
              <select
                value={summaryRole}
                ...
                className="border border-gray-300 rounded p-1 sm:p-2 w-full sm:w-auto text-xs sm:text-sm text-[#003087]"
              >
                <option value="" disabled className="text-gray-500">Select Division</option>
                {uniqueRoles.map((role) => (
                  <option key={role} value={role} className="text-[#003087]">{role}</option>
                ))}
              </select>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4">
            <div className="bg-white p-3 sm:p-4 md:p-6 rounded-lg shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
              <div className="flex items-start justify-between mb-3 sm:mb-4">
                ...
                <button
                  onClick={() => { setViewReport(null); setViewingDepartmentReport(false); }}
                  className={`${ACTION_BTN} ${ICON_BTN_DANGER}`}
                  aria-label="Close"
                  title="Close"
                >
              ...
              <p className="text-gray-700 text-xs sm:text-sm md:text-base">
                <span className="font-medium text-[#003087]">Division:</span> {viewReport?.role}
              </p>
              <p className="text-gray-700 text-xs sm:text-sm md:text-base">
              ...
              {pendingRegistrations.length === 0 ? (
                ...
                <div className="overflow-x-auto max-h-60">
                  <table className="w-full border-collapse table-auto text-xs sm:text-sm">
                    <thead>
                      <tr className="bg-[#003087] text-white sticky top-0">
                        <th className={`${TH_BASE} w-[50px]`}>No.</th>
                        <th className={`${TH_BASE} text-left`}>Name</th>
                        <th className={`${TH_BASE} text-left`}>Division</th>
                        <th className={`${TH_BASE} text-left`}>Office</th>
                        <th className={`${TH_BASE} text-left`}>Email</th>
                        <th className={`${TH_BASE} w-[150px]`}>Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {pendingRegistrations.map((r, index) => (
                        <tr key={r.id} className="border-b hover:bg-[#F5F6F5]">
                          <td className="p-1 sm:p-2 text-center text-[#003087]">{index + 1}</td>
                          <td className="p-1 sm:p-2 text-[#003087]">
                            <span className="inline-block max-w-[26ch] truncate" title={r.name}>{r.name}</span>
                          </td>
                          <td className="p-1 sm:p-2 text-[#003087]">{r.role}</td>
                          <td className="p-1 sm:p-2 text-[#003087]">
                            <span className="inline-block max-w-[24ch] truncate" title={r.office}>{r.office}</span>
                          </td>
                          <td className="p-1 sm:p-2 text-[#003087]">
                            <span className="inline-block max-w-[26ch] truncate" title={r.email_address}>{r.email_address}</span>
                          </td>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4">
            <div className="bg-white p-3 sm:p-4 md:p-6 rounded-lg shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
              <div className="flex items-start justify-between">
                ...
                <button
                  onClick={() => { setShowCategoryDeleteModal(false); setCategoryToDelete(null); }}
                  className={`${ACTION_BTN} ${ICON_BTN_DANGER}`}
                  aria-label="Close"
                  title="Close"
                >
      ...
      {/* Floating Action Cluster: Activity Logs + Users + Admin (no overlap) */}
      <div className="fixed bottom-[env(safe-area-inset-bottom,16px)] right-[env(safe-area-inset-right,16px)] z-40 flex flex-col gap-2">
        {/* Activity Logs FAB */}
        <button
          onClick={() => setActiveSlider(activeSlider === 'activity' ? null : 'activity')}
          className="rounded-full shadow-lg bg-white border border-gray-200 p-3 hover:bg-[#F5F6F5]"
          aria-label="Activity logs"
          title="Activity logs"
        >
          {/* activity/pulse icon */}
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#003087">
        ...
        >
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#C1272D">
            <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
          </svg>
        </button>
      </div>
      ...
      {/* Right Sliders */}
      {activeSlider === "activity" && (
        <div className="fixed top-0 right-0 h-full w-[260px] sm:w-[320px] bg-white shadow-xl transform transition-transform duration-300 ease-in-out z-40 translate-x-0">
          <div className="p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm sm:text-base font-semibold text-[#003087] font-['Poppins']">Activity Logs</h3>
              <button
              ...
              {recentActivity.length === 0 ? (
                ...
                <div key={a.id} className="border border-gray-200 rounded-md p-2">
                  <div className="text-xs text-[#003087] font-medium">
                    {truncateLabel(a.type_of_record, 40)}
                  </div>
                  <div className="text-[11px] text-gray-600">
                    {a.role} • {truncateLabel(a.period_covered || '', 30)}
                  </div>
      ...
      {activeSlider === "users" && (
        <div className="fixed top-0 right-0 h-full w-[250px] sm:w-[300px] bg-white shadow-xl transform transition-transform duration-300 ease-in-out z-40 translate-x-0">
          <div className="p-3 sm:p-4">
            <div className="flex justify-between items-center mb-3 sm:mb-4">
              <h2 className="text-base sm:text-lg font-semibold text-[#003087] font-['Poppins']">Manage Users</h2>
              <button onClick={() => setActiveSlider(null)} className={`${ACTION_BTN} ${ICON_BTN_DANGER}`} aria-label="Close" title="Close">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M18 6L6 18M6 6l12 12"/></svg>
              </button>
            </div>
            <div className="space-y-3 sm:space-y-4">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-[#003087] hover:text-[#002060] focus:outline-none text-xs sm:text-sm"
                >
              ...
              {showUserDetailsModal && selectedUser && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4">
                  <div className="bg-white p-3 sm:p-4 md:p-6 rounded-lg shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
                    <div className="flex justify-between items-center mb-3 sm:mb-4">
                      <h2 className="text-base sm:text-lg md:text-xl font-bold text-[#003087] font-['Poppins']">User Details</h2>
                      <button
                    ...
                    <p className="text-gray-700 text-xs sm:text-sm md:text-base">
                      <span className="font-medium text-[#003087]">Username:</span> {selectedUser.username}
                    </p>
                    <p className="text-gray-700 text-xs sm:text-sm md:text-base">
                      <span className="font-medium text-[#003087]">Division:</span> {selectedUser.role}
                    </p>
                    <p className="text-gray-700 text-xs sm:text-sm md:text-base">
                      <span className="font-medium text-[#003087]">Name:</span> {selectedUser.name || "N/A"}
                    </p>
                    <p className="text-gray-700 text-xs sm:text-sm md:text-base">
                      <span className="font-medium text-[#003087]">Office:</span> {selectedUser.office || "N/A"}
                    </p>
                    <p className="text-gray-700 text-xs sm:text-sm md:text-base">
                      <span className="font-medium text-[#003087]">Office Head:</span> {selectedUser.office_head || "N/A"}
                    </p>
                    <p className="text-gray-700 text-xs sm:text-sm md:text-base">
                      <span className="font-medium text-[#003087]">Email:</span> {selectedUser.email_address || "N/A"}
                    </p>
                    <button
                      onClick={() => setShowUserDetailsModal(false)}
                      className="mt-3 sm:mt-4 w-full py-1 sm:py-2 bg-[#003087] text-white rounded hover:bg-[#002060] transition-colors font-medium text-xs sm:text-sm"
      ...
      {activeSlider === "admin" && (
        <div className="fixed top-0 right-0 h-full w-[250px] sm:w-[300px] bg-white shadow-xl transform transition-transform duration-300 ease-in-out z-40 translate-x-0">
          <div className="p-3 sm:p-4">
            <div className="flex justify-between items-center mb-3 sm:mb-4">
              <h2 className="text-base sm:text-lg font-semibold text-[#003087] font-['Poppins']">Manage Admin</h2>
              <button onClick={() => setActiveSlider(null)} className={`${ACTION_BTN} ${ICON_BTN_DANGER}`} aria-label="Close" title="Close">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M18 6L6 18M6 6l12 12"/></svg>
              </button>
            </div>
